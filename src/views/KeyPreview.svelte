<script lang="ts">
  import { color_set } from "../lib/store";
  const dt = [
    ["adj_curves", "CURVES"],
    ["adj_huesaturation", "HUESAT"],
    ["adj_exposure", "EXPSRE"],
    ["adj_colorbalance", "COLBAL"],
    ["adj_gradientmap", "GRDMAP"],
    ["adj_lut", "LUT"],
  ];
  ...
  $: {
    if (value) {
      switch (value.data_mode) {
        case "macro":
          if (value.data_macro) {
            const key = value.data_macro
              .filter((e: any) => e.down)
              .map((e: any) => e.key);
            keys = key;
            v = value.macro_title;
  ...
  export let which_layer = 0;
  let whichcolor = ["#fd0", "#fff"];

  $: {
    whichcolor = color_set[which_layer];
  }
</script>

<div
  style="background-color:{whichcolor[0]};color:{whichcolor[1]};"
  class={`text-[0.6rem] font-oswald  select-none overflow-clip pointer-events-none justify-center items-center flex font-black uppercase`}
>
  {#if is_mod}
    <div>
      <svg
  ...
  {:else}
    <div>
      {#if value.data_mode == "action"}
        <div class="text-xs">
          {v.replaceAll(/^adj_/g, "").replace(/[aioeoAIUEO]/g, "")}
        </div>
      {:else if value.data_mode == "macro"}
        <div class="flex flex-row">
          <div class="flex flex-col justify-center items-center">
            <div class="text-xs">{v}</div>
            <div class="text-[8px] font-normal leading-none">
              {keys.join(" ")}
            </div>
          </div>
        </div>
