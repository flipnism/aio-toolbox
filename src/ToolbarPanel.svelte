<script lang="ts">
  ...
  const dontIdle = () => {
    appWindow.isFocused().then((result) => {
      if (!result) appWindow.setFocus();
    });
    opacity = 1;
    clearTimeout(delayIdle);
  ...
  function saveAppConfig() {
    writeTextFile("app_config.json", JSON.stringify(app_configs, null, 2), {
...
{#if app_configs}
  <div style="opacity: {opacity};" class="transition-all bg-base-300/90">
    ...
    >
      <div class="text-center text-[.7rem] font-black p-[5px]">
        {mouse_location}
      </div>

      <div
        style="width:{progress * 1}px;height:{progress *
          1}px;opacity:{progress / 5 / 100};"
